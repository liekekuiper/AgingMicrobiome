...
def find_complete(metadata, model, subset, out):
    ...
    for factor_name in factor_names:
        if factor_name == 'sex':
            final_df.loc[:,'sex'] = pd.Categorical(final_df['sex'], categories=['men', 'women'])
            ...
            final_df.loc[:,factor_name] = pd.Categorical(final_df[factor_name], categories=[0, 1])
...
def process(taxonomy, tree, feature_table, output, threads, metadata, model, sub, out):
    # Load metadata
    try:
    ...
    meta_df = find_complete(meta, model, sub, out)
    ...
    string_list = list(map(str, sample_ids))
    common_sample_ids = [sid for sid in string_list if sid in valid_sample_ids]
