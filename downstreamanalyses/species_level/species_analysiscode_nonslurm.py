...
factors = factors_str.split(',')  # Split the string into a list
...
for subs in subsets:
    ...
    for out in outcomes:
        for model in models:
            ...
            model_terms = model.split('+')  # Split the model string into a list of terms
            datafile = process(
                taxonomy=taxonomy,
                tree=tree,
...
results_df.to_csv(f"Results_species_{cohort}_{label}_{pd.Timestamp.today().date()}.csv", index=False)
